    ...
    return logAudit(
      {
        ...
        suggestions: {
          ...prev.suggestions,
          [attractionId]: { ...s, cost: { ...s.cost, ...cost } },
        },
      },
      "Cost updated",
...
export function setBackupTags(attractionId: string, tags: string[]) {
  setState((prev) => {
    ...
    return {
      ...prev,
      suggestions: { ...prev.suggestions, [attractionId]: { ...s, backupTags: tags } },
    };
  });
}
...
export function recordSponsorDecision(
  ...
  setState((prev) => {
    ...
    const updated = s ? recordDecision(next, attractionId, status, note ? { note } : {}) : next;
    return withNotice(
...
export function setRoles(travellerId: string, roles: TripRole[]) {
  setState((prev) => {
    ...
    if (owners.length === 1 && owners[0]!.id === travellerId && !roles.includes("owner"))
      return prev;
...
export function addNeed(travellerId: string, need: Omit<TravellerConstraint, "id">) {
  setState((prev) => {
    if (!isOrganiser(prev) && !canEditFor(prev, travellerId)) return prev;
    return {
      ...prev,
...
export function publishItinerary(acknowledged: string[]) {
  setState((prev) => {
    ...
    const next: KintripState = {
      ...
      publishedChanges: record
        ? [record, ...(prev.publishedChanges ?? [])]
        : (prev.publishedChanges ?? []),
    };
...
export function splitDay(
  ...
  setState((prev) => {
              ...
              groups: [
                {
                  ...
                  label: "Main group",
                  memberIds: rest,
                  activity: "Stays with the planned day",
                },
                {
                  id: uid(),
                  label: input.label || "Second group",
    ...
    return withNotice(logAudit(next, "Day split", `Day ${dayNumber}: ${input.activity}`), {
      text: `Day ${dayNumber} splits into two groups, meeting at ${input.meetingPoint} at ${input.meetingTime}`,
      audience: "all",
    });
  });
}
...
export function rejoinDay(dayNumber: number) {
  setState((prev) => {
    if (!prev.itinerary || !isOrganiser(prev)) return prev;
    const days = prev.itinerary.days.map((d) =>
      d.day === dayNumber ? { ...d, split: undefined } : d,
    );
    return logAudit(
...
export function assignHelper(travellerId: string, managerId: string) {
  setState((prev) => {
        ...
        : {
            ...
            consentLog: [
              ...(t.consentLog ?? []),
              {
                at: new Date().toISOString(),
                action:
                  adult && prev.activeTravellerId !== travellerId
                    ? ("assigned" as const)
                    : ("accepted" as const),
                by: actorName(prev),
              },
    ...
    return logAudit(
      { ...prev, travellers },
      "Help assigned",
      `${target.name} — ${actorName(prev, managerId)}`,
    );
  });
}
...
export function castVote(travellerId: string, attractionId: string, value: VoteValue) {
  setState((prev) => {
    ...
    const next = {
      ...
      votes: {
        ...prev.votes,
        [travellerId]: { ...(prev.votes[travellerId] ?? {}), [attractionId]: value },
      },
    };
...
export function addNeedFor(travellerId: string, need: Omit<TravellerConstraint, "id">) {
  setState((prev) => {
    ...
    return proxy
      ? logAudit(next, "Need entered on behalf", `${proxyLabel(prev, travellerId)}: ${need.label}`)
      : next;
  });
}
...
export function addBooking(
  input: Omit<Booking, "id" | "createdAt" | "version" | "referenceSharedWith"> & {
  ...
  setState((prev) => {
    ...
    return withNotice(
      logAudit({ ...prev, bookings: [booking, ...prev.bookings] }, "Booking added", booking.title),
      {
...
export function deleteBooking(id: string) {
  setState((prev) => {
    ...
    if (!isOrganiser(prev) && prev.activeTravellerId !== existing.ownerId) return prev;
    return logAudit(
      { ...prev, bookings: prev.bookings.filter((b) => b.id !== id) },
      "Booking removed",
      existing.title,
    );
  });
...
export function addTask(input: {
  ...
  setState((prev) => {
    ...
    return task.assigneeId
      ? withNotice(
          logAudit(next, "Job assigned", `${task.title} → ${actorName(prev, task.assigneeId)}`),
          {
      ...
      t.id !== taskId
        ...
        : accept
          ? {
              ...
              version: t.version + 1,
              history: [taskEvent(prev, "Accepted"), ...t.history],
            }
          : {
              ...t,
    ...
    const tasks = prev.tasks.map((t) =>
      t.id !== taskId
        ...
        : {
            ...
            version: t.version + 1,
            history: [taskEvent(prev, "Cancelled"), ...t.history],
          },
    );
...
export function addPackingItem(input: {
  ...
  setState((prev) => {
    if (input.scope !== "shared" && input.travellerId && !canEditFor(prev, input.travellerId))
      return prev;
 ...
 */
export function commitPackingQuantity(
  itemId: string,
  amount: number,
    ...
    const packing = prev.packing.map((p) =>
      p.id === itemId
        ? {
            ...
            responsibleId: amount > 0 ? prev.activeTravellerId : p.responsibleId,
            version: p.version + 1,
          }
        : p,
    );
...
export function setPackingPacked(itemId: string, packed: boolean) {
  setState((prev) => {
    const item = prev.packing.find((p) => p.id === itemId);
    if (!item) return prev;
    if (item.scope !== "shared" && item.travellerId && !canEditFor(prev, item.travellerId))
      return prev;
    const packing = prev.packing.map((p) =>
      p.id === itemId
        ? {
            ...
            packed,
            quantityPacked:
              p.scope === "shared" ? (packed ? (p.quantityCommitted ?? 0) : 0) : p.quantityPacked,
            version: p.version + 1,
          }
        : p,
    );
...
export const PACKING_TEMPLATES: Record<
  string,
  { label: string; scope: PackingItem["scope"]; quantityNeeded?: number }[]
> = {
...
export function applyPackingTemplate(name: keyof typeof PACKING_TEMPLATES) {
  setState((prev) => {
    ...
    return logAudit(
      { ...prev, packing: [...prev.packing, ...items] },
      "Packing template added",
      String(name),
    );
  });
}
  ...
  for (const day of after) {
    ...
    for (const item of day.items) {
      ...
      if (!prevItem) {
        changes.push({
          kind: "added",
          text: `Day ${day.day}: ${item.title} was added at ${item.start}`,
          dayNumber: day.day,
          material: true,
          affected: [],
        });
        ...
        changes.push({
          kind: "moved",
          text: `Day ${day.day}: ${item.title} moved from ${prevItem.start} to ${item.start}`,
          dayNumber: day.day,
          material: true,
          affected: [],
        });
    ...
    for (const prevItem of old.items) {
      if (!newItems.has(prevItem.id)) {
        changes.push({
          kind: "removed",
          text: `Day ${day.day}: ${prevItem.title} is no longer in the plan`,
          dayNumber: day.day,
          material: true,
          affected: [],
        });
...
export function setAttendance(
  ...
  setState((prev) => {
    if (!canEditFor(prev, travellerId)) return prev;
    const rest = (prev.attendance ?? []).filter(
      (a) => !(a.travellerId === travellerId && a.itemId === itemId),
    );
